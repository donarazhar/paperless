...
<div class="compose-wrapper">
    ...
    <div class="compose-topbar">
        ...
        <h1><i class="bi bi-pencil-square me-2" style="color:#6366f1;"></i>Edit Surat</h1>
    </div>

    <!-- Body -->
    <div class="compose-body">
        @php
            $defaultSubject      = old('subject', $letter->subject);
            $defaultBody         = old('body', $letter->body);
            $defaultType         = old('letter_type', $letter->type);
            $defaultToUnit       = old('to_unit_id', $letter->to_unit_id);
            $defaultExtRecipient = old('external_recipient_name', $letter->external_recipient_name);
            $defaultLetterNum    = old('letter_number', $letter->letter_number == '-' ? '' : $letter->letter_number);
            $encodedId           = \Vinkla\Hashids\Facades\Hashids::encode($letter->id);
        @endphp

        <form action="{{ route('letters.update', $encodedId) }}" method="POST"
              enctype="multipart/form-data" id="editForm" style="width:100%;max-width:680px;">
            @csrf
            @method('PUT')
            ...
            <div class="compose-card">
                ...
                <div class="compose-field" style="flex-direction: column; align-items: flex-start; gap: 0; padding-bottom: .5rem;">
                    <span class="compose-field-label" style="width: auto; margin-bottom: .5rem;">Lampiran Saat Ini:</span>
                    <div class="file-chips" style="margin-top: 0;">
                        @foreach($letter->attachments as $att)
                            @php
                                $ext = strtolower(pathinfo($att->file_path, PATHINFO_EXTENSION));
                                $iconClass = 'doc'; $iconBi = 'file-earmark-word-fill';
                                if($ext === 'pdf') { $iconClass = 'pdf'; $iconBi = 'file-earmark-pdf-fill'; }
                                elseif(in_array($ext, ['jpg','jpeg','png'])) { $iconClass = 'img'; $iconBi = 'file-earmark-image-fill'; }
                            @endphp
                            <div class="file-chip {{ $iconClass }}">
                                <i class="bi bi-{{ $iconBi }}"></i><span class="chip-name">{{ $att->file_name }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="compose-attach">
                    ...
                    <div class="field-note mt-1">
                        <i class="bi bi-info-circle"></i>
                        Biarkan kosong jika tidak ingin mengubah lampiran yang ada. Mengunggah lampiran baru akan menggantikan yang lama.
                    </div>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function updateFileInputAndUI() {
            ...
            if (allFiles.length > 0) {
                ...
                label.textContent = allFiles.length + ' file PDF baru dipilih (Bisa tambah file lagi)';
