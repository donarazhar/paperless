...
<div class="page-container">
    <div class="inbox-header">
        <div>
            <h1 class="ih-title">Kotak Masuk</h1>
            <div class="ih-sub">Pusat penerimaan surat masuk internal & eksternal</div>
        </div>
        <div style="background: #eef2ff; color: #4f46e5; padding: 0.5rem 1rem; border-radius: 10px; font-weight: 700; font-size: 0.85rem;">
            <i class="bi bi-inbox-fill me-2"></i>{{ $letters->total() }} Pesan
        </div>
    </div>
    ...
    <div class="mail-toolbar">
        <div class="d-flex align-items-center gap-1">
            ...
            <button class="tb-btn" title="Tandai Sudah Dibaca"><i class="bi bi-envelope-open"></i></button>
    ...
    <div class="mail-list">
        @forelse($letters as $letter)
            @php
                $isUnread = in_array($letter->status, ['pending_agenda', 'pending_approval']);
                $showUrl = route('letters.show', ['letter' => \Vinkla\Hashids\Facades\Hashids::encode($letter->id)]);
                
                $isExternal = $letter->type === 'external';
                $senderName = $isExternal ? ($letter->external_sender_name ?? 'Unknown') : ($letter->sender->unit->name ?? ($letter->sender->name ?? 'Unknown'));
        ...
        @empty
            <div class="empty-state">
                <i class="bi bi-inbox-fill" style="background: linear-gradient(135deg, #e2e8f0, #cbd5e1); -webkit-background-clip: text; -webkit-text-fill-color: transparent;"></i>
                <div class="empty-title">Kotak Masuk Kosong</div>
                <div class="empty-desc">Belum ada surat yang masuk atau diteruskan ke kotak masuk Anda saat ini. Anda dapat bersantai!</div>
            </div>
        @endforelse
    </div>
</div>
